...
class opencl_py:
	...
	def __init__(self,platform,func):
		...
		if func=="aes":filen="pbkdf2-sha1_aes-256-cbc.cl"
		elif func=="concat":filen="concat_s.cl"
		elif func=="str32":filen="b32.cl"
		elif func=="eqs":filen="eqs.cl"
		elif func=="aes32":filen="pbkdf2-sha1_aes-256-cbc.cl"
		elif func=="aes32_concat":filen="pbkdf2-sha1_aes-256-cbc.cl"


		self.OPENCL_CODE_PATH=os.path.join("Library","worker","generic",filen)
	...
	def run_aes(self,hashkey,crypted_data,printspeed=False):
		mf = cl.mem_flags# opencl memflag enum
		nAESblocks=np.int32(len(crypted_data)/16)

		hashk=np.array(hashkey,dtype=np.void)
		hash_g = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=hashk)

		crypted_data = [crypted_data]#*len(hashkey)
		cryp =np.array(crypted_data,dtype=np.void)
		data_g = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=cryp)
		...
		result = np.empty(hashk.shape, dtype=cryp.dtype)
		...
		finish_event=self.prg.func_pbkdf2(self.queue,gdim,None, hash_g, data_g, result_g,nAESblocks)
		finish_event.wait()
...
#			print("OpenCL Speed: "+str(password_step/1e-9/(finish_event.profile.END-start_event.profile.START)/1000)+" K passphrase/s")
		rt = cl.enqueue_copy(self.queue, result, result_g)# copy the result from device to host,type of "result" is a list of unsigned integer(32bit,4byte)
	...
	def run_concat(self,str1,str2,printspeed=False):
		...
		str1_g = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=npstr1)

		npstr2=np.array(str2,dtype=np.void)
		str2_g = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=npstr2)
		...
		finish_event=self.prg.concat_str(self.queue,npstr1.shape,None, str1_g, str2_g, result_g)
		finish_event.wait()
		#******************Call Kernel****************** ,if set localsize (512,) to None,the runtime will automatically takes care of block/grid distribution
...
#			print("OpenCL Speed: "+str(password_step/1e-9/(finish_event.profile.END-start_event.profile.START)/1000)+" K passphrase/s")
	...
	def run_substr(self,str1,lens,printspeed=False):
		...
		finish_event=self.prg.concat_str(self.queue,npstr1.shape,None, str1_g, result_g,strlen)
		finish_event.wait()
		rt = cl.enqueue_copy(self.queue, result, result_g)# copy the result from device to host,type of "result" is a list of unsigned integer(32bit,4byte)
	...
	def run_aes32(self,str1,lens,crypted_data,printspeed=False):
		...
		result = np.empty(npstr1.shape, dtype=np.dtype('V'+str(lens)))
		result_s = cl.Buffer(self.ctx, mf.WRITE_ONLY,result.nbytes)# size should be in byte, 1byte=8bit; notice that in python, bool=8bit=1byte

		start_event=cl.enqueue_marker(self.queue)

		finish_event=self.prg.substr(self.queue,npstr1.shape,None, str1_g, result_s,strlen)
		finish_event.wait()
		...
		finish_event=self.prg.func_pbkdf2(self.queue,gdim,None, result_s, data_g, result_g,nAESblocks)
		finish_event.wait()
		#******************Call Kernel****************** ,if set localsize (512,) to None,the runtime will automatically takes care of block/grid distribution
...
#			print("OpenCL Speed: "+str(password_step/1e-9/(finish_event.profile.END-start_event.profile.START)/1000)+" K passphrase/s")
	...
	def run_aes32_concat(self,str1,lens,crypted_data1,crypted_data2,printspeed=False):
		...
		def aes32(npstr1,lens,crypted):
			...
			data_g = cl.Buffer(self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=crypted)
		...
		finish_event=self.prg.concat_str(self.queue,npstr1.shape,None, result_g1, result_g2, result_g)
		finish_event.wait()
