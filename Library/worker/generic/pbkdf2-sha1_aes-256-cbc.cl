...
//----------------------------------------------Kernel-------------------------------------------------------
__kernel void func_pbkdf2(
    __global uchar * key_g,  
    __global uchar * oridata, 
    __global uchar * result,
    const unsigned int naesblocks
    )
{
    ...
    uchar key[AES_CBC_256_KEY_BYTE_SIZE] ;

    uint ggid;
    ggid=(gid/naesblocks);
    #pragma unroll AES_CBC_256_KEY_BYTE_SIZE
    for(uint i=ggid*AES_CBC_256_KEY_BYTE_SIZE;
                i<((ggid+1)*AES_CBC_256_KEY_BYTE_SIZE);//+AES_CBC_256_KEY_BYTE_SIZE;
                i++){
            key[p]=key_g[i];
            p++;
        }
    ...
    p=0;
    uint he=(gid%naesblocks)*ENC_DATA_BLOCKSIZE;//0,16,32...
    #pragma unroll ENC_DATA_BLOCKSIZE
    for(uint i=(he);i<(he)+ENC_DATA_BLOCKSIZE;i++)
    {
        data[p]=oridata[i];
        p++;
    }
    p=0;
    int hep=((gid%naesblocks)-1)*ENC_DATA_BLOCKSIZE;
    #pragma unroll ENC_DATA_BLOCKSIZE
    for(int i=(hep);i<ENC_DATA_BLOCKSIZE+(hep);i++)
    {
        if (i<0){ivs[p]=0x00;}
        else{ivs[p]=oridata[i];}
        p++;
    }
    ...
    p=0;
    he=gid*ENC_DATA_BLOCKSIZE;
    #pragma unroll ENC_DATA_BLOCKSIZE
    for (uint i=(he);i<ENC_DATA_BLOCKSIZE+(he);i++)
        {
            result[i]=data[p];
            p++;
    }
